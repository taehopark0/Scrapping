...
def tokenize(doc):
    ...
    return new_list
data = [tokenize(d) for d in morphed_title_list]
print(data)
new_title_split_list=[title_split_list]
# 문장을 이용하여 단어와 벡터를 생성한다.
#size=M size, window=중심단어+주변단어 갯수, 단어가 최소 한 번 이상 반복되는 것들만 고려하겠다.
size=len(title_split_list)
model = word2vec.Word2Vec(data,size=3179,window=5,min_count=1)
#model.build_vocab(morphed_title_list) # trainset을 넣는
model.train(morphed_title_list, epochs=model.iter, total_examples=model.corpus_count)
...
#result_sim=model.wv.most_similar(positive=tokenize('생리 예정일'),negative='',topn=5)
#[('시간', 0.9999356269836426), ('야즈', 0.9999312162399292), ('장기', 0.99992835521698), ('응급', 0.9999250769615173), ('주기', 0.9999090433120728)]
#result_sim2=model.most_similar(positive=u'질',negative='',topn=10)
